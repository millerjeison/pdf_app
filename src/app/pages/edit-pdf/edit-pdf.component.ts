...
export class EditPdfComponent {
  ...
  async renderPdf() {
    ...
    try {
      ...
      console.log(pdfDocument.getFieldObjects());
  ...
  addInputToCanvas(x: number, y: number, width: number, height: number, inputType?: string) {
  ...
  async addMetadataAndSave(): Promise<void> {
    try {
      ...
      const link = document.createElement('a');
      link.href = URL.createObjectURL(blob);
      link.download = this.fileName + "_modified.pdf";
      link.click();
    } catch (error) {
      console.error('Error al procesar el PDF:', error);
    }
